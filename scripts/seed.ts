// Idempotent seed: clear-and-reseed inside one transaction. Re-running yields
...
async function main() {
  ...
  const [cats, prods, variants, images] = await Promise.all([
  ...
  console.log(`  categories=${cats} products=${prods} variants=${variants} images=${images}`);
}
